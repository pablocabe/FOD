...
M = 572 / 68
M = 8
En un nodo del arbol entrarían 7 registros Persona, al ser un arbol B de orden 8.

c. ¿Qué impacto tiene sobre el valor de M organizar el archivo con toda la
información de los alumnos como un árbol B?

El valor de M determina la cantidad máxima de claves (M-1) y de hijos (M) que puede tener un nodo en el árbol B. Un valor mayor de 
M resultará en nodos más grandes y, por lo tanto, en una estructura de árbol B más ancha y menos profunda. 
Por otro lado, un valor menor de M resultará en nodos más pequeños y en una estructura de árbol B más profunda pero más estrecha.
...
Los datos que seleccionaría como clave de identificación para organizar los elementos en el árbol B serían el DNI 
o el legajo ya que ambos son campos únicos para los alumnos de la Facultad de Informática.
...
En el peor de los casos, se necesita de h lecturas (con h altura del árbol).
...
Si se desea buscar un alumno por un criterio diferente se debe tener en cuenta el árbol por completo, 
siendo necesarias n lecturas en el peor de los casos, siendo n la cantidad total de nodos que hay en el árbol.
}